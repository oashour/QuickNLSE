...
********************************************************************************/
static int max_index(double *arr, int size)
{
	int largest_Index = 0;
...
void cu_max_psi(double *d_R, double *d_I, double *max, int step, int size)
{
	...
	cudaMemcpy(h_R, d_R, sizeof(double) * size, cudaMemcpyDeviceToHost);
	cudaMemcpy(h_I, d_I, sizeof(double) * size, cudaMemcpyDeviceToHost);
